          ...
          return {
            ...
            scans: mappedScans.map((scan: any) => ({
              ...
              metrics: {
                ...
                codeSmells: scan.metrics?.codeSmells ?? scan.metrics?.code_smells ?? 0,
                coverage: scan.metrics?.coverage ?? 0,
                debtRatio: scan.metrics?.debtRatio ?? scan.metrics?.debt_ratio ?? 0,
                duplicatedLinesDensity: scan.metrics?.duplicatedLinesDensity ?? scan.metrics?.duplicated_lines_density ?? 0,
                ...
                securityHotspots: scan.metrics?.securityHotspots ?? scan.metrics?.security_hotspots ?? 0,
                securityRating: scan.metrics?.securityRating ?? scan.metrics?.security_rating ?? '',
                technicalDebtMinutes: scan.metrics?.technicalDebtMinutes ?? scan.metrics?.technical_debt_minutes ?? 0,
        ...
        return {
          ...
          scans: logicalScans.map((s: any) => ({
            ...
            metrics: s.metrics ? {
              ...
              vulnerabilities: s.metrics.vulnerabilities ?? s.metrics.vulnerabilities ?? 0,
              codeSmells: s.metrics.codeSmells ?? s.metrics.code_smells ?? 0,
              coverage: s.metrics.coverage ?? 0,
              debtRatio: s.metrics.debtRatio ?? s.metrics.debt_ratio ?? 0,
              duplicatedLinesDensity: s.metrics.duplicatedLinesDensity ?? s.metrics.duplicated_lines_density ?? 0,
              securityHotspots: s.metrics.securityHotspots ?? s.metrics.security_hotspots ?? 0,
              technicalDebtMinutes: s.metrics.technicalDebtMinutes ?? s.metrics.technical_debt_minutes ?? 0,
